...
var HeaderView = Backbone.View.extend({
    ...
    events: {
        ...
        //CONTEXT MENU
        'click .contextMinimize': 'minimize',
        'click .contextMaximize': 'maximize',
        'click .contextItem.contextClose': 'exit',
    },
    ...
    initialize: function(todos) {
        try {
            ...
            Ipc.on('blur-main-window', this.blur);
    ...
    addTodo: function(evt) {
        ...
        if (evt.keyCode && evt.keyCode == 13 || evt.type == "click") {
            ...
            if (todoItem && todoItem.toString().trim()) {
    ...
    import: function() {
        Ipc.send('import-data-from-file');

    },
    export: function() {
        Ipc.send('export-data-from-file', localStorage.getItem('todoApp'));
    ...
    fullscreen: function() {
        Ipc.send('set-fullscreen-main-window');
