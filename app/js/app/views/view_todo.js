...
var RootView = Backbone.View.extend({
    ...
    initialize: function() {
        try {
            ...
            //this.todos.on("change:type_id reset add remove", this.updateProgressBar, this);
            this.listenTo(this.todos, "reset add remove", this.updateProgressBar);
            ...
            PubSub.on('todo:updated', this.updateProgressBar, this);


            Ipc.on('atom-context', function(ctx) {
                console.log(ctx)
            });
            ...
            PubSub.trigger('todo:initiated', 'welcome user')
        }
    },
    ...
    updateProgressBar: function() {
        debugger;
        var completed = this.todos.where({
            type_id: 2 // completed
        }).length / this.todos.length;

        Ipc.send('update-progress-bar', completed);
